...
| `operator.startingCSV` | string | `cert-manager-operator.v1.19.1` | **Set this to the version you want running, and keep it current.** It does two jobs: OLM uses it as the STARTING point on a fresh install (not a ceiling — OLM offers newer versions afterwards), and the approver treats it as the only version it will approve. Those diverge if the cluster ends up newer, in which case the approver **warns and continues** — it approves nothing, the install proceeds, and the log tells you to adopt what is running, since OLM cannot downgrade and blocking the release would not correct the pin. A newer catalog serves a newer head: OpenShift 4.20 offers `v1.20.0` on `stable-v1` where 4.18 offers `v1.19.1`. Note the CSV name does not begin with `packageName`. **Must be `<package>.v<numeric version>`** — rendering fails otherwise. Not because of the version comparison, which normalises each side to its bare version and handles a `v`-less name fine, but because **approval matches CSV names byte-for-byte**: the search loop compares this against `.spec.clusterServiceVersionNames` and the verify step uses it as a resource name, so a spelling the catalog does not publish classifies correctly forever, approves nothing, and burns the Job's whole budget instead of failing at render. A `-rc.1` suffix is refused for a measured reason of its own: `sort -V` is not semver-aware and orders a pre-release *after* the release it precedes, so an rc pin against its own installed final reads as an upgrade that can never arrive. Copy the name from `oc get packagemanifest`. |
